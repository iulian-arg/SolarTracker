...
private:
    ...
    static std::deque<String> logQueueLongTerm;
    ...
    static void log(const char *level, const char *tag, const char *fmt, va_list args)
    {
        ...
        if (logQueueLongTerm.size() >= maxQueueSize)
        {
            logQueueLongTerm.pop_front(); // remove oldest
        }
        logQueueLongTerm.push_back(logEntry);
    }
};
